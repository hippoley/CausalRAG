...
[**Live verified replay**](https://hippoley.github.io/CausalRAG/) · [**Verified examples**](https://hippoley.github.io/CausalRAG/examples.html) · [**Quickstart**](#60-second-quickstart) · [**Production contract**](docs/production-execution-contract.md) · [**Capability packs**](docs/capability-packs.md)
...
| **plug into an OpenAI Agents approval flow** | [OpenAI Agents adapter](docs/openai-agents.md) |
| **protect any framework over HTTP** | [Execution gateway](docs/execution-gateway.md) |
| **expose the boundary to MCP hosts** | [MCP execution boundary](docs/mcp.md) |
...
```text
Branchpoint doctor 0.3.0: PASS
  OK        decision_runtime: proposer=submit_form runtime=inspect_submission_state
  OK        authorization: unauthorized principal denied before handler
  OK        durable_receipt: same effect id replayed stored result; handler executed once
```

Then install the UI extra and open the local workbench:
...
The Branchpoint workbench does not hard-code one domain.
...
## One production boundary, end to end

The no-key reference composes runtime arbitration, principal authorization, downstream idempotency, and durable receipts in one flow:

```bash
python examples/production_execution_boundary.py
```

It proves that a proposer can put `submit_form` first while Branchpoint verifies state first, that an authorized operator can cross the side-effect boundary, that the same effect id does not submit twice, and that an unauthorized guest never reaches the handler.

See the [production execution contract](docs/production-execution-contract.md) for guarantees and non-guarantees.

---

## Authorization is not model confidence

Tool permissions are deployment-owned policy.

```python
from branchpoint import AuthorizationContext, ToolSpec

resident = AuthorizationContext.from_permissions(
    "resident:alice",
    ["home.window.open"],
)
...
A proposer may suggest `open_bedroom_window` with high confidence. It cannot grant itself `home.window.open` by putting a permission string in its arguments. Authorization is resolved immediately before tool execution, and expired authority fails closed.

For multi-user or voice systems, pass a dynamic `authorization_resolver(state, action)` so the current speaker/session principal is checked at consequence time rather than copied into a prompt.

---

## Durable sessions and execution receipts

For local or single-node deployments, Branchpoint can persist **session artifacts** and **effect receipts** in SQLite. Multi-instance execution receipts can use PostgreSQL.

```bash
export BRANCHPOINT_PROBE_SESSION_DB=./data/probe-sessions.sqlite3
```

The probe archive stores the latest world snapshot, pending Human Gate, trajectory ledger, operator interactions, telemetry, and final result. After a process restart it is available as a read-only recovery artifact.

For effectful tools, use `SQLiteExecutionLedger` and stable effect ids. Completed effects replay their stored observation; an unknown crash-window effect fails closed until the real-world outcome is reconciled. Branchpoint does **not** claim that a local receipt store alone can make an arbitrary external side effect exactly-once.
...
pip install -e ".[postgres]"          # multi-instance durable receipts
pip install -e ".[openai-agents]"     # OpenAI Agents approval/execution adapter
pip install -e ".[mcp]"               # MCP v2 consequence boundary
...
If an ordinary agent loop takes the wrong branch, open a [Failure case](https://github.com/hippoley/CausalRAG/issues/new?template=failure-case.yml). If you already have a reproducible world + tools + hypotheses, propose a [Capability pack](https://github.com/hippoley/CausalRAG/issues/new?template=capability-pack.yml). If you built something public with Branchpoint, share it through the [Showcase](https://github.com/hippoley/CausalRAG/issues/new?template=showcase.yml).
