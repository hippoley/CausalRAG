...
class OutcomeLikelihood:
    """Likelihood model for one mutually exclusive experiment outcome."""
...
class ExperimentContract:
    """Runtime-owned discrete observation model for mutually exclusive hypotheses.

    The contract defines P(outcome | hypothesis, action). For each hypothesis,
    likelihoods across outcomes must sum to one. This is enough to calculate an
    expected entropy reduction before acting and an exact posterior after an
    outcome is observed.
    """
    ...
    def __post_init__(self) -> None:
        ...
        for outcome in self.outcomes:
            if set(outcome.likelihoods) != set(hypothesis_ids):
                raise ValueError(
                    "every outcome must provide likelihoods for the same hypotheses"
                )
        ...
        for hypothesis_id in hypothesis_ids:
            ...
            if abs(total - 1.0) > self.tolerance:
                raise ValueError(
                    f"likelihoods for {hypothesis_id} must sum to 1; got {total}"
                )

    def hypothesis_ids(self) -> List[str]:
        if not self.outcomes:
            return []
        return [str(value) for value in self.outcomes[0].likelihoods.keys()]
...
def _normalized_prior(
    ...
    for hypothesis_id in contract.hypothesis_ids():
        ...
        if hypothesis is None or hypothesis.status == "rejected":
            raw[hypothesis_id] = 0.0
        else:
            raw[hypothesis_id] = max(0.0, float(hypothesis.probability))
...
def posterior_for_outcome(
    ...
    """Compute normalized posterior P(H | observed outcome, action)."""
    prior = _normalized_prior(contract, world_model)
    numerators = {
        hypothesis_id: prior_probability
        * contract.likelihood(outcome, hypothesis_id)
    ...
    if evidence_probability <= 0.0:
        return prior
    return {
        hypothesis_id: numerator / evidence_probability
        for hypothesis_id, numerator in numerators.items()
    }
...
def expected_information_gain(
    ...
    """Return expected fractional entropy reduction in [0, 1]."""
    prior = _normalized_prior(contract, world_model)
...
def apply_experiment_observation(
    contract: ExperimentContract,
    world_model: CausalWorldModel,
    observation: Any,
    source: str,
    metadata: Optional[Dict[str, Any]] = None,
) -> Optional[ExperimentUpdate]:
    """Apply an observed contract outcome directly to hypothesis credences."""
    ...
    for hypothesis_id, probability in posterior.items():
        ...
        evidence = Evidence(
            source=source,
            statement=(
                f"Experiment {contract.experiment_id} observed outcome '{outcome}': "
                f"{hypothesis_id} {previous:.4f} -> {probability:.4f}"
            ),
        ...
        world_model.set_hypothesis_probability(
            hypothesis_id,
